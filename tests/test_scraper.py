...
def test_run_scraper_creates_output(tmp_path, monkeypatch):
    ...
    assert rendered_path.endswith("_contractor.png") or rendered_path.endswith("_contractor_dentist.png") or rendered_path.endswith("_contractor_realtor.png")
...
def test_automatic_regeneration_on_weak_quality(tmp_path, monkeypatch):
    ...
    class FakePage:
        ...
        def screenshot(self, path, full_page):
            image = Image.new("RGB", (10, 10), color=(10, 10, 10))
            image.save(path, format="PNG")
    ...
    if result["quality_label"] == "needs improvement" or result["vision_label"] == "needs improvement":
