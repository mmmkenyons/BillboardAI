...
def verify() -> int:
    ...
    try:
        ...
        check("Summary counts", summary.total_prospects == 9)
        check("Active/closed counts", summary.active_prospects == 6 and summary.closed_prospects == 3)
        check("Won/lost/not interested counts", summary.won_prospects == 1 and summary.lost_prospects == 1 and summary.not_interested_prospects == 1)
        check("Stage breakdown", all(summary.stage_counts[s] == 1 for s in PIPELINE_WORKFLOW_ORDER))

        urgent_store = ProspectStore(path=os.path.join(root, "urgent.json"))
        for prospect in [
            ...
            Prospect(prospect_id="closed", workflow_status=WORKFLOW_STATUS_WON, priority=PRIORITY_URGENT, next_action_date="2026-08-14"),
        ]:
            urgent_store.create(prospect)
        urgent_store.save()
        urgent_summary = ProspectPipelineService(urgent_store).summary(today=today)
        check("Needs-attention overdue", urgent_summary.overdue_prospects == 1)
        check("Needs-attention due today", urgent_summary.due_today_prospects == 1)
        check("Needs-attention urgent", urgent_summary.urgent_prospects == 1)
        check("Needs-attention excludes closed", urgent_summary.closed_prospects == 1 and urgent_summary.urgent_prospects == 1)
        ...
        print("[FAIL] Unexpected verifier exception")
        traceback.print_exc()
        failed += 1
